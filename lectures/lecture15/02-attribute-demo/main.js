/* 
cat:  https://cdn.mos.cms.futurecdn.net/VSy6kJDNq2pSXsCzb6cvYF.jpg
dog:  https://kb.rspca.org.au/wp-content/uploads/2018/11/golder-retriever-puppy.jpeg
bird: https://upload.wikimedia.org/wikipedia/commons/thumb/4/45/Eopsaltria_australis_-_Mogo_Campground.jpg/1200px-Eopsaltria_australis_-_Mogo_Campground.jpg
fish: https://thumbs-prod.si-cdn.com/n7Z82GD9Eav_CtpnzizNo66-dKc=/420x240/https://public-media.si-cdn.com/filer/d6/93/d6939718-4e41-44a8-a8f3-d13648d2bcd0/c3npbx.jpg'
*/


function showCat() {
    // your code here...
    console.log('update the image to show a cat!');
    document.querySelector('#image-demo').src="https://cdn.pixabay.com/photo/2017/02/20/18/03/cat-2083492__340.jpg"
};

function showDog() {
    // your code here...
    console.log('update the image to show a dog!');
    document.querySelector('#image-demo').src="https://cdn.pixabay.com/photo/2017/09/25/13/12/puppy-2785074__340.jpg"
};

function showBird() {
    // Target the id and then change the src to an image of a bird
    console.log('update the image to show a bird!');
    document.querySelector('#image-demo').src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQaIMxeq6HuHvHDQ5RVdqPBtb0ffQfBPhpAr5uBksCt3g&s"
};

function showFish() {
    // your code here...
    console.log('update the image to show a fish!');
    document.querySelector('#image-demo').src="https://media.cnn.com/api/v1/images/stellar/prod/220309153016-02-rose-veiled-fairy-wrasse-discovery.jpg?c=16x9&q=w_800,c_fill"
};

